...
const initiatePayment = async (req, res) => {
  ...
  const ticketCount = registrationDetails?.ticketCount || 1;
  const calculatedAmount = event.ticketPrice * ticketCount;
  ...
  res.status(200).json({ transaction_uuid, amount: calculatedAmount });
};

const verifyPaymentAndRegister = async (req, res) => {
  const { transaction_uuid } = req.body;

  const pending = await PendingRegistration.findOne({ transaction_uuid });
  if (!pending) {
    res.status(404);
  ...
  const emailHtml = `
    <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.1); border: 1px solid #e2e8f0;">
      <div style="background-color: #5CB85C; padding: 40px 20px; text-align: center;">
        <h1 style="color: white; margin: 0; font-size: 32px; letter-spacing: 3px; font-weight: 900;">EVENTIFY</h1>
        <p style="color: rgba(255,255,255,0.9); margin-top: 10px; font-weight: 500;">Payment & Registration Confirmed!</p>
      </div>
      
      <div style="padding: 40px; color: #1e293b;">
        <h2 style="font-size: 24px; font-weight: 700; margin-bottom: 20px; color: #0f172a;">Hello ${recipientName},</h2>
        <p style="font-size: 16px; line-height: 1.6; margin-bottom: 30px;">Your payment was successful and you are now registered for the following event. We're excited to have you join us!</p>
