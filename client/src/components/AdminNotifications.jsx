...
export default function AdminNotifications() {
  ...
  const fetchNotifications = useCallback(async ({ quiet = false } = {}) => {
    ...
    try {
      ...
      // Keep notification polling quiet so it does not disturb admin workflows.
    } finally {
  ...
  const openNotification = async (notification) => {
    ...
    if (notification.event) {
      navigate(`/admin-view-details/${notification.event}`);
    }
  };
          ...
          <div className="max-h-[440px] overflow-y-auto">
            {loading ? (
              ...
              notifications.map((notification) => {
                const config = typeConfig[notification.type] || typeConfig.event_registered;
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs font-bold text-slate-400">
                        ...
                        <span>|</span>
                        <span>{notification.userEmail || notification.userName}</span>
                        <span>|</span>
